...
scfa_indv_qc <- scfa_indv %>% 
  mutate(acetate_mmol = round(acetate_mM, digits = 2)) %>%
  mutate(butyrate_mmol = round(butyrate_mM, digits = 2)) %>%
  mutate(propionate_mmol = round(propionate_mM, digits = 2)) %>%
  rename(race_ethnicity = `race/ethnicity`) %>% 
  mutate(ht_m = round((ht_in / 35.274), digits=1)) %>% 
  mutate(wt_kg = wt_lbs / 2.205) %>% 
  select(-ht_in, -wt_lbs, -acetate_mM, -butyrate_mM, -propionate_mM) %>%
  filter(notes != "frozen >24hrs") %>% 
  filter(sample_wt_g > 0.10) %>% 
  mutate(sample_wt_kg = sample_wt_g / 1000) %>%
  mutate(acetate_mmol_kg = round((acetate_mmol / sample_wt_kg), digits = 2)) %>%
  mutate(butyrate_mmol_kg = round((butyrate_mmol / sample_wt_kg), digits = 2)) %>%
  mutate(propionate_mmol_kg = round((propionate_mmol / sample_wt_kg), digits = 2)) %>%
  mutate(scfa_total = round((propionate_mmol + butyrate_mmol + acetate_mmol), digits = 2))
```
...
scfa_wkly_qc <- read_excel("raw_data/SCFA_data.xlsx", sheet = "SCFA_wkly", col_names = TRUE, trim_ws = TRUE, na = c("", "NA", "-----")) %>%
  ...
  filter(study_week == "week1" |study_week ==  "week2" |study_week ==  "week3") %>%
  mutate(mean_scfa = ((acetate_mean + butyrate_mean + propionate_mean) / 3)) %>%
  mutate(median_scfa = ((acetate_median + butyrate_median + propionate_median)/3))
```
