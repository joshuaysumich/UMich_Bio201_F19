...
Now that the data are in long format, calculate the weekly mean for each SCFA per participant. Make use of the code you've already written, just swap out the variables for the data frames and column names. Group by study week and SCFA and then get mean of the concentration 
```{r}
# copy+paste and edit code here
#I added in supplement consumed for the next part, but I don't think it belongs there.
...
```{r}
# wkly_means shouldn't have supplement consumed?
p1 <- wkly_means %>%
...
ggplot2::ggsave(filename = "total_scfa_plot",
                ...
                dpi = "retina")
ggplot2::ggsave(filename = "Mean_Wkly_SCFAs",
                path = "Lab4/curated_data/",
                device = "tiff",
                plot = p1,
                dpi = "retina")
...
pH_qc <- pH_data %>%
  filter(!sample_id %in% pH_bad$sample_id) %>% 
  group_by(study_week, participant_id) %>% #it's unclear to me what to include, so I chose what I thought was important. I can come back and put in ID in formation if necessary
...
BSS_qc <- BSS_data %>%
  ...
  summarize(mean_bristol_numeric = mean(bristol_numeric, na.rm = TRUE)) #calculate weekly average 
dim(BSS_qc)
```
...
pH_BSS_wkly <- full_join(pH_qc, BSS_qc,
                    by = c()) %>%
  mutate(mean_bristol_numeric = signif(mean_bristol_numeric, digits = 3)) %>% #round avgs to reflect sig figs
  mutate(mean_ph = signif(mean_ph, digits = 3)) #round avgs to reflect sig figs
```
