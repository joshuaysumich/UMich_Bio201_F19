...
sv_table <- read_csv("~/UMich_Bio201_F19/Lab7/raw_data/seq_variants_wide.csv", 
    na = "NA") %>%
...
taxa <- read_csv("~/UMich_Bio201_F19/Lab7/raw_data/seqvar_classifications.csv", 
    na = "NA") %>%
...
```{r}
#I raised the necessary abundance to account for the larger number of species types. I feel like 1% is still pretty exclusive consiidering some species clearly dominate
...
physq_long_genus <- physq_sub %>%
  ...
  select(-sample_number, -quantity_compliant, -use_data, 
         -ends_with("_mm"), -ends_with("_mmol_kg"), 
         -starts_with("bristol"), -ph) %>% #drop columns not using
  filter(abundance > .1) %>% #remove low abund taxa
  arrange(genus) #sort alphabetically by taxa

bar_phys_gen_plot<-ggplot(data = physq_long_genus, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
  ...
  scale_fill_manual(name = "Genus", #change name for legend
                    values = taxa_colors) #indicate colors from vector of custom colors 
                    #current legend labels
ggplot2::ggsave(filename = "bar_phys_gen_plot", "~/UMich_Bio201_F19/Lab8/figures/",
                device = "tiff",
                plot = bar_phys_gen_plot,
                dpi = "retina")
bar_phys_gen_plot
...
physq_long_order <- physq_sub %>%
  ...
  select(-sample_number, -quantity_compliant, -use_data, 
         -ends_with("_mm"), -ends_with("_mmol_kg"), 
         -starts_with("bristol"), -ph) %>% #drop columns not using
  filter(abundance > .01) %>% #remove low abund taxa
  arrange(order) #sort alphabetically by taxa

bar_phys_ord_plot<-ggplot(data = physq_long_order, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
  ...
  theme(axis.text.x = element_text(angle = 90,
                                   vjust = 0.5)) + #clean up x-axis labels 
  ylab("Relative Abundance (order > 0.01%)") +
  scale_fill_manual(name = "order", #change name for legend
                    values = taxa_colors) #indicate colors from vector of custom colors 
                    #current legend labels

ggplot2::ggsave(filename = "bar_phys_ord_plot", "~/UMich_Bio201_F19/Lab8/figures/",
                device = "tiff",
                plot = bar_phys_ord_plot,
                dpi = "retina")
bar_phys_ord_plot
```
...
physq_j <- ordinate(physq_sub, 
                      method = "NMDS", 
                      k=2, maxit=120, try=50,
                      distance = "jaccard")
physq_j
#I got the lowest stress I could at 120 maxit, as stress = 0.226
```
...
```{r}
jac_ord_plot<- plot_ordination(physeq = physq_sub, 
                     ordination = physq_j, 
                     type = "samples", 
                     color = "semester", 
                     shape = "study_week") +
...
ggplot2::ggsave(filename = "jac_ord_plot", "~/UMich_Bio201_F19/Lab8/figures/",
                device = "tiff",
                plot = jac_ord_plot,
                dpi = "retina")
...
# run test
adn_res_jac <- adonis(formula = dat_jac ~ semester * study_week, 
                  data = samplebf)
...
Values: semester - R2=.029, p = .001
study_week - R2 = .0077, p = .001

Interpretation: Similar to the previous test. It has a low p-value, indicating a significant difference, but also has a low R2 indicating that the grouping of samples is weak. This suggests that the variation of samples in our tested groups is influencing our results.
